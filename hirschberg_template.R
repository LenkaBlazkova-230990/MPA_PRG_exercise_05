...
HirschbergTemplate <- function(X, Y, align, match, mismatch, gap){
    ...
    if (length(X)==0){ # length of X is equal to zero
        for (i in 1:length(Y)){ # for each character in Y
            first_align_row <- append(first_align_row, DNAString('-')) # add gap
            second_align_row <- append(second_align_row, Y[i]) # add character from Y
        }
        # align <- list(DNAStringSet(first_align_row), DNAStringSet(second_align_row))
        ...
        for (i in 1:length(X)){ # for each character in X
            first_align_row <- append(first_align_row, X[i]) # add character from X
            second_align_row <- append(second_align_row, DNAString('-')) # add gap
        }
        # align <- list(DNAStringSet(first_align_row), DNAStringSet(second_align_row))
        align <- c(DNAStringSet(first_align_row), DNAStringSet(second_align_row))
        print(align)
    ...
    else
    {
        ...
        ymid <- which.max(left_score + rev(right_score)) - 1

        # The first half
        if (ymid==0){ # index of division for Y is equal to 0
            align <- HirschbergTemplate(X[1:xmid], DNAString(), align, match, mismatch, gap)
            # call Hirschberg function for the first half of X and for an empty DNAString object
        }else{
            align <- HirschbergTemplate(X[1:xmid], Y[1:ymid], align, match, mismatch, gap)
            # call Hirschberg function for the first half of X and for the first part of Y
        }
        
        # The second half
        if ((xmid + 1) > xlen){ # X cannot be further divided
            align <- HirschbergTemplate(DNAString(), Y[(ymid+1):ylen], align, match, mismatch, gap)
            ...
            align <- HirschbergTemplate(X[(xmid+1):xlen], DNAString(), align, match, mismatch, gap)
            # call Hirschberg function for the second half of X and for an empty DNAString object
        }else{
            align <- HirschbergTemplate(X[(xmid+1):xlen], Y[(ymid+1):ylen], align, match, mismatch, gap)
...
HirschbergTemplate(X=DNAString('AGTACGCA'), Y=DNAString('TATGC'), align=list(DNAString(''),DNAString('')), match=2, mismatch=-1, gap=-2)